...
This repo packages [LNDg](https://github.com/cryptosharks131/lndg) for StartOS.
...
LNDg runs the `ghcr.io/cryptosharks131/lndg` image pinned via `dockerTag` in `startos/manifest/index.ts`.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `ghcr.io/cryptosharks131/lndg:v<new version>`.
