...
- **Hard dependency on LND, reached over the LXC bridge.** `main.ts` resolves LND's gRPC `host:port` from LND's `grpc` host/interface (looked up with `sdk.host.get({ hostId: 'grpc', packageId: 'lnd' })`) and writes it into `LND_RPC_SERVER`. The `'grpc'` ids are **string literals** — this repo does not declare `lnd-startos` as a source dependency, so the ids can't be imported from it; keep them in sync with lnd-startos by hand. TLS is validated against `tls.cert` read off the read-only LND mount at `/mnt/lnd`, whose StartOS-issued cert covers the bridge address.
