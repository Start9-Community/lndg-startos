...
| `LND_RPC_SERVER` | Override | LND's gRPC `host:port` over the LXC bridge, resolved reactively from the `lnd` dependency's `grpc` host at start. Replaces the old `lnd.startos:10009` DNS name — LND's StartOS-issued cert now covers the bridge address, which the `tls.cert` on the read-only LND mount validates. |
